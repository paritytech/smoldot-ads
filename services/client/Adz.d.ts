...
declare type MoveFirstArgumentToTheEnd<T extends (...args: any) => any> = T extends (...args: infer A) => infer R ? (...args: MoveFirstToTheEnd<A>) => R : unknown;
export declare type PolkaQueryFunction<A extends any[], T extends any[]> = MoveFirstArgumentToTheEnd<(cb: (...args: T) => void, ...args: A) => Promise<() => void>>;
declare type SignAndSendFn<T> = {
    (address: string, options: {
        signer: Signer;
    }, cb: (update: T) => void): Promise<() => void>;
    (keyring: KeyringPair, cb: (update: T) => void): Promise<() => void>;
};
export interface SignAndSend<T> {
    signAndSend: SignAndSendFn<T>;
}
export declare type PolkaMutation<A extends any[], R = ISubmittableResult> = (...args: A) => SignAndSend<R>;
...
export interface AdzQueries {
    numOfAds: PolkaQueryFunction<[], [numOfAds: {
        toString: () => string;
    }]>;
...
export declare type AdzApi = ApiPromise & {
    tx: ApiPromise["tx"] & {
        adz: AdzMutations;
    };
